...
namespace pure
{
    ...
    struct Renderer : private NonCopyable
    {
        ...
        PURE2D_API void drawBuffer(uint32_t start, uint32_t count, VertexBuffer buffer, const Texture *texture,
                                   const Shader& shader, const ElementBuffer* ebo = nullptr, DrawPrimitive primtype = DrawPrimitive::TRIANGLE_STRIP);
		PURE2D_API void drawBuffer(uint32_t start, uint32_t count, VertexBuffer buffer, const Texture *texture,
        ...
        PURE2D_API void drawQuad(const Quad& quad, const Mat4* transform, const Shader& shader, const Texture* texture = nullptr);
